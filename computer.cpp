...
double computer::alpha_beta(int depth, double alpha, double beta,int father,bool color,int p1,int p2)
{
	...
	else if (m_manager->judge_success((m_color+1)%2))
	{
		...
		if (m_manager->get_mode()) {
			int bx(0), by(0);
			int b_data(0), c_data(0);
			bool temp_color = 0;
			double ret(0);
			bool cut(0);
			temp_color = m_manager->moving_color();
			int i;
			for (i = 1,i+=(bool(i) == (color)); i <= m_manager->m_chess_num; i += 2) {
				data_itoxy(m_manager->m_chess[i], bx, by);
				if (m_manager->move_select(bx, by)) {

					b_data = data2real(data_xytoi(bx, by));
					m_current_state.first |= 1 << b_data;
					m_current_state.first |= temp_color << b_data;
					for (int x = 1; x <= board_size; x++)
					{
						for (int y = 1; y <= board_size; y++)
						{
							if (m_manager->move_step(x, y)) {
								...
								m_current_state.first |= temp_color << c_data;
			...
			for (int x = 1; x <= board_size; x++)
			{
				...
				for (int y = 1; y <= board_size; y++)
				{
					...
					if (m_manager->step(x, y)) {
						...
						m_current_state.first |= temp_color << data;
